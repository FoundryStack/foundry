defmodule SparkMeta.WalkerTest do
  ...
  describe "walk/1 generic fields" do
    ...
    test "existing DslState fields still present" do
      {:ok, state} = SparkMeta.Walker.walk(MockResource)
      ...
      assert Map.has_key?(state, :moduledoc)

      # Ash-specific fields are now in extension_data, not at top level
      refute Map.has_key?(state, :attributes)
      refute Map.has_key?(state, :relationships)
      refute Map.has_key?(state, :actions)
      refute Map.has_key?(state, :policies)
      refute Map.has_key?(state, :compliance)
      refute Map.has_key?(state, :telemetry_prefix)
    end
  end
end
