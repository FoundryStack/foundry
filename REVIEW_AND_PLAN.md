...
| 68 | Phase 5 (Auto-Apply) was a standalone phase with no new infrastructure | Closed | Eliminated as a phase. `auto_apply_classes` manifest key + two-line guard merged into Phase 4 deliverables. Phases renumbered 6→5, 7→6, 8→7. BUILD_SEQUENCE.md updated. |
| 69 | Five named speckit postures (analyze/plan/clarify/checklist/constitution) added spec surface area with no testable behavioral difference | Closed | Collapsed to single pre-generation checklist in AGENTS.md §Spec-Kit Tasks. Checklist items preserved verbatim. Five-name taxonomy removed. |
| 70 | max_tool_calls default of 8 too low for cross-domain changes | Closed | Raised to 20 in ADR-010 §Circuit Breaker and manifest example. Normal operations use 8–15 calls; 20 prevents silent truncation on complex tasks. |
| 71 | Reasoning trace required JSON block teaches model to fill fields, not reason | Closed | ADR-010 §Reasoning Trace replaced: tool call log is the trace; `intent_classification` float emitted in telemetry span only. Dev-mode `copilot_trace.jsonl` retained for debugging. |
| 72 | Four confidence states (HIGH/MEDIUM/LOW/BLOCKED) overcomplicated behavioral model | Closed | ADR-013 §Confidence collapsed to two behavioral paths (proceed / ask+surface gap) plus BLOCKED. Confidence emitted as float in telemetry span; no named state in code. |
| 73 | CHANGE_PREVIEW structured field requirements created a second format contract to maintain | Closed | ADR-013 §Phase-Gated simplified to plain prose description. No structured field requirements. |
| 74 | Question response 1–3 sentence limit incorrect for complex questions | Closed | ADR-013 §Response Format: sentence limit removed. Citation requirement retained. |
| 75 | Spec-kit documents for governed changes lived outside the proposal system (human commits manually) | Closed | INV-002 updated. ADR-013 §speckit response format updated. ADR-014 §DRAFT updated. All files (ADR, tests, code, migration) generated in one pass on proposal branch. Approver reviews and approves everything in one diff. Human never touches git for governed changes. |
| 76 | No visualization preview of proposed structural changes on canvas | Closed | ADR-016 §Proposal Preview Mode added. `Foundry.Proposals.GraphDelta` struct defined. `graph_delta` field added to proposal JSON (ADR-014). Canvas renders phantom nodes (dashed, amber) from plan confirmation time. No subprocess needed — derived from operation parameters. |
| 77 | Spec-first generation order unspecified — agent could generate code before tests | Closed | AGENTS.md agent reasoning sequence updated to 12 steps. Generation order within proposal branch: spec-kit files first, then test skeletons (as specification format), then implementation, then migration. Agent runs `mix test` once as pre-surface quality gate; max 3 self-corrections at compile level. |
