defmodule IgamingRef.Finance.LedgerEntry do
  ...
  attributes do
    ...
    attribute :amount, :money do
      description "The monetary amount of this movement. Always positive — direction is conveyed by the :direction attribute."
      allow_nil? false
    end
    ...
    attribute :reference_id, :string do
      description "External reference identifier — e.g. the WithdrawalRequest ID or provider transaction reference."
      allow_nil? true
      constraints max_length: 255
    end
  ...
  identities do
    identity :unique_idempotency_key, [:idempotency_key] do
      description "Prevents duplicate ledger entries from retry storms. The idempotency key is the Transfer's idempotency key scoped to this movement."
    end
  end
  ...
  actions do
    ...
    create :record do
      description "Append a new ledger entry. Called exclusively by Transfer modules — never called directly by application code."
