defmodule IgamingRef.Promotions.BonusCampaign do
  ...
  actions do
    ...
    create :create do
      ...
      accept [:name, :kind, :eligibility_rule, :bonus_amount, :wagering_multiplier,
              :max_redemptions, :starts_at, :expires_at]
    end

    update :update do
      description "Update campaign configuration. Only permitted while in :draft state."
      accept [:name, :bonus_amount, :wagering_multiplier, :max_redemptions, :starts_at, :expires_at]
  ...
  policies do
    policy action_type(:read) do
      description "All authenticated users may read campaign details (terms must be transparent — RG-MGA-005)."
