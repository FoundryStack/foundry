defmodule IgamingRef.Gaming.ProviderSyncReactor do
  ...
  step :load_provider do
    ...
    run fn %{provider_id: pid}, _ ->
      case Ash.get(ProviderConfig, pid, actor: :system) do
        {:ok, config} when config.status == :active -> {:ok, config}
        {:ok, config} -> {:error, "Provider #{pid} is not active (status: #{config.status})"}
        {:error, err} -> {:error, err}
      end
    end
  end
  ...
  step :sync_games do
    ...
    run fn %{provider_id: pid, games: game_list}, _ ->
      results = Enum.map(game_list, fn game_data ->
