defmodule Foundry.Context.Scenarios.FlowExpander do
  @moduledoc false

  alias Foundry.Context.Scenarios.Adapters.Ash
  alias Foundry.Context.Scenarios.Adapters.Oban
  alias Foundry.Context.Scenarios.Adapters.Reactor
  alias Foundry.Context.Scenarios.Adapters.Rule
  alias Foundry.Context.Scenarios.Adapters.Trigger
  alias Foundry.Context.Scenarios.FlowSummary

  @adapters [Rule, Reactor, Trigger, Oban, Ash]
  ...
  def maybe_expand_automatic_runtime_steps(steps, lookup) do
    Enum.flat_map(steps, fn step ->
      if Map.get(step, :capture_origin) == "automatic" do
        expand_step(step, lookup)
      else
        [step]
      end
    end)
