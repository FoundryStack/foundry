...
- **MCP tools and resources are auto-discovered via DCR** — on session start, agents read `.well-known/mcp.json` to get the server URL and DCR endpoint, then POST to `/register` with `client_name` to obtain a Bearer token. All subsequent tool calls and resource reads use that token. Available tools: `project_status`, `module_context`, `system_graph`, `run_lint`, `read_doc`, `submit_proposal`, `proposal_status`, `edit_file`. Available resources: `agents_guide` (AGENTS.md), `adr_index`, `runbooks`, `build_sequence`, `implementation_summary`, `lint_catalogue`. Do not re-read `.well-known/mcp.json` after initialization — store the token and reuse it.
...
- **Always call multiple tools in parallel when exploring code.** Return multiple function calls in a single response — never call one tool and wait for a result before deciding the next tool. For example: explore a directory tree by calling `list_directory` for all known subdirectories at once; read related files by calling `read_file` for all of them in one response. A single response with 3–5 tool calls is always preferred over 3–5 sequential single-tool responses.
