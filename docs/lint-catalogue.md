...
### LiveView State Coverage (ADR-033)

| Rule ID | Severity | Description | Implementation module | Status |
|---|---|---|---|---|
| `:missing_liveview_state_coverage` | `:warning` | A Phoenix LiveView or LiveComponent module's `render/1` function does not handle all required interaction states. Required: `@loading` assign branch, `@error` assign branch, and at least one empty-collection guard (`Enum.empty?`, `== []`, `length(` check). Missing state handling causes blank or broken UI in production and violates the pre-generation checklist added by ADR-033. Components generated by AshPyro are exempt. | `Foundry.LintRules.LiveViewStateCoverageRule` | implemented |
